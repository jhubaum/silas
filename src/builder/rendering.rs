...
impl<'a> OrgHTMLHandler<'a> {
    ...
    fn write_link <W: Write>(&mut self, w: &mut W, link: &elements::Link) -> Result<bool, HTMLExportError> {
        ...
        if link_type.is_none() {
            println!("Warning: Link {} in {:?} has no type. It will not be resolved", link.path, self.post.unwrap().path);
            return Ok(true);
        }
        ...
        match link_type.unwrap() {
            ...
            lt => {
                println!("Warning: Unknown link type {} in file {:?}. Link will not be resolved", lt, self.post.unwrap().path);
                return Ok(true);
            }
        };
        Ok(false)
...
impl HtmlHandler<HTMLExportError> for OrgHTMLHandler<'_> {
    fn start<W: Write>(&mut self, mut w: W, element: &Element) -> Result<(), HTMLExportError> {
        match element {
            ...
            Element::Link(link) => if self.write_link(&mut w, &link)? {
