...
use handlebars::{
    Handlebars, RenderContext, Helper,
    Context, JsonRender, HelperResult,
    Output, TemplateFileError
};
...
impl<'a> Theme<'a> {
    pub fn load(path: &'a str) -> Result<Self, ThemeError> {
        ...
        Ok(Theme { templates, theme_dir: path })
    }
    ...
    pub fn render<TData: Serialize>(&self, file: &mut File, template: &str, data: &TData) -> Result<(), RenderError> {
